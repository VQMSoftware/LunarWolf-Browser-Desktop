...
export class ITab {
  ...
  public constructor(
    ...
    // Initialize with zero width and animate in
    this.width = 0;
    this.left = this.getLeft(true);
    ...
    // Animate tab in after creation
    requestAnimationFrame(() => {
      this.setWidth(this.getWidth(), true);
      this.setLeft(this.getLeft(true), true);
  ...
  public async select() {
    if (!this.isClosing) {
      ...
      // Ensure smooth transition when selecting
      this.setWidth(this.getWidth(), true);
      this.setLeft(this.getLeft(true), true);
    }
  }
